  ...
  return (
    <div>
      <div className="weather__card col-lg-6 mx-auto mt-3">
        <h4>
          {result.city_name}, {result.country_code}
        </h4>
        <span className="flex-div">
          <img
            src={`${process.env.REACT_APP_ICON_URL}${result.weather.icon}.png`}
          ...
          <p className="weather__temperature">
            {topTemp === "C"
              ? result.temp
              : (Math.round(result.temp) * 9) / 5 + 32}
            &deg;
          </p>
          <span className="unit__button">
            <button className="active" onClick={handleTempUnit} value={topTemp}>
              &nbsp;
              {topTemp}&nbsp;
            </button>
            <button onClick={handleTempUnit} value={bottomTemp}>
              &nbsp;
              {bottomTemp}&nbsp;
            </button>
          </span>
        ...
        <p className="weather__time">
          Updated as of {new Date(result.ts * 1000).toLocaleTimeString()}
        </p>

        <span className="weather__widgets">
          <p>
            Feels Like {result.app_temp ? Math.round(result.app_temp) : "--"}
            &deg;
          </p>
          <p>
            Wind <i className="fas fa-location-arrow"></i>&nbsp;
            {result.wind_spd ? Math.round(result.wind_spd * 3.6) : "--"} m/s
          </p>
